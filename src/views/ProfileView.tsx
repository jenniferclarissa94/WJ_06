...
import { Heart, MessageSquare, Settings, Bell, LogOut, Camera } from 'lucide-react';
...
export default function ProfileView({ navigateDetail }: { navigateDetail?: (type: string, id: string) => void }) {
  ...
  return (
    <div className="pb-24 pt-6 px-4 max-w-md mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-2">
        ...
        <button onClick={() => navigateDetail && navigateDetail('notifications', 'me')} className="w-full flex items-center justify-between p-4 rounded-xl bg-[var(--color-dark-surface)] hover:bg-[var(--color-dark-surface-2)] transition border border-white/5">
          ...
          <div className="text-gray-400">
            <span className="text-xs uppercase tracking-wide font-bold mr-2">Manage</span>
          </div>
        </button>
      ...
      <div className="pt-4">
        <button className="w-full flex items-center justify-center gap-2 p-4 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500/20 transition border border-red-500/20 font-bold">
          <LogOut size={18} />
          Log Out
        </button>
      </div>
